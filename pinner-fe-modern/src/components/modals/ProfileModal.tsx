...
export default function ProfileModal() {
  ...
  async function deleteTraveler() {
    if (window.confirm('정말 탈퇴하실건가요?')) {
      let resultStatus = await apiv1
        .post('/traveler/delete', JSON.stringify(traveler))
        .then((response) => {
          alert(response.data);
  ...
  async function deleteOauthTraveler() {
    if (window.confirm('정말 연동해제(탈퇴)하실건가요?')) {
      let resultStatus = await apiv1
        .post('/traveler/delete/afteroauth', JSON.stringify(traveler))
        .then((response) => {
          alert(response.data);
          return response.status;
        })
  ...
  const onSubmit = async (event) => {
    ...
    let pwResult = await apiv1
      .post('/traveler/password/check', nowPasswordData)
      .then((response) => {
        setErrorMessage('');
        return true;
      })
        ...
        {signupServices === 'Web' ? (
          ...
          <Box className={style.profile_box}>
            ...
            <div className={style.save_btn}>
              ...
              <Button color='primary' variant='solid' onClick={onSubmit} sx={{ width: '100px', marginLeft: 'auto' }}>
                저장
              </Button>
              <Button
                color='danger'
