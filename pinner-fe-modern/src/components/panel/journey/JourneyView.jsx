...
export default function JourneyView({ readOnly, travelId, journey, viewCancel }) {
  ...
  async function onCreate() {
    ...
    if (photos.length !== 0 && hasExceededSize(photos, maxPhotoSize)) {
      ...
      photos.length === 0 &&
      !window.confirm('사진이 없습니다. 그래도 저장하시겠습니까?\n(저장 후 수정으로 사진 추가가 가능합니다.)')
    ) {
      return;
    }
    ...
    try {
      ...
      await apiv1.put(`/journey/${journey.id}`, formData).then((response) => {
  ...
  const onHashTagChange = useCallback((e) => {
    let map = e.detail.tagify.value.map((e) => e.value);
    setHashtags(map);
  }, []);
    ...
    <>
      ...
      >
          ...
          <>
            {editMode === EditMode.EDIT ? (
              ...
              <div className={style.view_location}>
                <Typography sx={{ fontSize: '21px', fontWeight: 'bold' }}>{journey.geoLocationDto.name}</Typography>
              </div>
            )}
          </>
          <>
            {editMode === EditMode.EDIT ? (
              <div className={style.edit_country_date}>
                <Autocomplete
                  ...
                  renderInput={(params) => <TextField {...params} placeholder='국가를 선택해주세요.' />}
                />
              ...
              <div className={style.view_country_date}>
                <Typography>{dayjs(journey.date).format('YYYY.MM.DD')}</Typography>
              </div>
            )}
          </>
          <>
            {editMode === EditMode.EDIT ? (
              ...
              <div className={style.view_tags}>
                {journey.hashtags.map((tag, index) => (
                  <div key={index} className={style.view_tag}>
                    {tag}
                  </div>
                ))}
              </div>
            ...
            <div className={style.edit_tool}>
              ...
              <label className={style.add_icon}>
                <Tooltip title='사진 추가' variant='outlined' size='lg'>
                  <AddBoxOutlinedIcon sx={{ fontSize: '30px' }} onClick={() => inputRef.current.click()} />
                </Tooltip>
              </label>
        ...
        <Box className={style.journeyView_imageBox}>
          {editMode === EditMode.EDIT ? (
            <>
              {saving && <CircularProgress className={style.saving_icon} size={48} />}
              {photos.length > 0 ? (
                    ...
                    return (
                      <ImageListItem key={index}>
                        <ImageListItemBar
                          ...
                          actionPosition='left'
                          actionIcon={
                            <IconButton sx={{ color: 'red' }} onClick={() => removePhoto(index)}>
      ...
      {lightBoxOpen && <LightBoxPill photo={lightBoxPhotos} open={lightBoxOpen} setOpen={setLightBoxOpen} />}
    </>
  );
}
