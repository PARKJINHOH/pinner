...
export default function NewJourneyPill({ travel, editingCancel }) {
  ...
  async function onCreate() {
    ...
    try {
      ...
      await apiv1.post(`/journey`, formData).then((response) => {
    ...
    if (files && files.length > 0) {
      ...
      for (const newPhoto of newPhotos) {
        ...
        if (data) {
          ...
          await apiv1
            .get('/geocoding', { params: { lat: data.lat, lng: data.lng, reverse: true } })
            .then((response) => {
  ...
  function ButtonField(props) {
    const { setOpen, label, id, disabled, InputProps: { ref } = {} } = props;
