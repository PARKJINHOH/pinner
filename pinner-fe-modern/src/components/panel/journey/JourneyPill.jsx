...
export default function JourneyPill({ travelId, editMode, setEditMode, journey }) {
  ...
  async function onDeleteClick() {
    if (window.confirm(`"${journey.geoLocationDto.name}" 여정을 정말 삭제하실건가요?`)) {
      await apiv1.delete(`/journey/${journey.id}`).then((response) => {
        setEditMode('');
  ...
  return (
    <>
      <Box className={style.root_box}>
        <div onClick={onJourneyViewClick}>
          {editMode === 'DELETE' && (
            ...
            >
              <DeleteForeverOutlinedIcon className={style.journey_delete_icon} sx={{ fontSize: 30, color: 'red' }} />
            </IconButton>
          )}
          <Box className={style.preview_box}>
            ...
            <div className={style.journey_info}>
              <Chip size='small' sx={{ backgroundColor: '#5b5b5b', color: 'white' }} label={journeyDate} />
              <Chip
                size='small'
                sx={{ backgroundColor: '#5b5b5b', color: 'white', marginLeft: '120px' }}
        ...
        {journey.hashtags.map((tag, index) => (
          <div key={index} className={style.journey_tags}>
            {tag}
          </div>
        ))}
      </Box>

      {
        ...
        isJourneyViewState && (
          <JourneyView travelId={travelId} journey={journey} viewCancel={() => setIsJourneyViewState(false)} />
        )
      }
    </>
  );
