...
export default function TravelPill({ travel, editMode, setEditMode }) {
  ...
  const newData = uniqueDate.reduce((acc, v) => [...acc, [...journeyList.filter((d) => d.date === v)]], []);
  ...
  function onJourneyClick() {
    ...
    const points = journeyList.map((j) => j.geoLocationDto).filter((point) => point.lat !== 0 && point.lat !== 0);
  ...
  const travelTitle = (
    <Box>
      <Typography sx={{ fontWeight: 'bold', fontSize: '19px' }}>{travel.title}</Typography>
    </Box>
  );
  ...
  async function onDeleteClick() {
    if (window.confirm(`"${travel.title}" 여행을 정말 삭제하실건가요?`)) {
      await apiv1.delete(`/travel/${travel.id}`).then((response) => {
  ...
  return (
    <>
      <Box className={style.root_box}>
        <div onClick={onJourneyClick}>
          {editMode === 'DELETE' && (
            ...
            >
              <DeleteForeverOutlinedIcon className={style.travel_delete_icon} sx={{ fontSize: 30, color: 'red' }} />
            </IconButton>
          )}
          <Box className={style.travel_box}>
            {photo !== null ? (
              ...
              <Typography color='textSecondary'>사진 없음</Typography>
            )}
...

      {
        ...
        selectedTravelId === travel.id && <JourneyList travel={travel} />
      }
    </>
  );
}
