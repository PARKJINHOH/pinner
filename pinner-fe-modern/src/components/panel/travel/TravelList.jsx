...
export default function TravelList() {
  ...
  const onDragEnd = (result) => {
    ...
    apiv1.put('/travel/orderKey', newTravelData).then((response) => {
      setTravelData(response.data);
    });
  };
  ...
  return (
    <Paper className={style.root_paper} sx={{ width: _travelListViewWidth, left: _sidebarWidth }}>
      <List className={style.sidebar_list_div}>
        {isLoggedIn ? (
            ...
            <div className={style.travel_tool}>
              ...
              <Tooltip title='여행 추가' variant='outlined' size='lg'>
                <AddBoxOutlinedIcon
                  ...
                  onClick={() => {
                    setEditMode((prevMode) => (prevMode === EditMode.ADD ? EditMode.DEFAULT : EditMode.ADD));
                    if (dndState === false) {
                      setDndState(true);
                    }
                    setSelectedId(null);
                      ...
                      <div {...provided.droppableProps} ref={provided.innerRef}>
                        {travelData.map((travel, idx) => {
                          return (
                            <Draggable
                              key={travel.orderKey}
                              draggableId={String(travel.orderKey)}
                              index={idx}
                              isDragDisabled={dndState}
                            >
                              {(provided) => (
                                <div
          ...
          <Box className={style.login_title_box}>로그인 후 이용해주세요. ✈</Box>
        )}
      </List>
    </Paper>
  );
